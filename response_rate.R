...
# Convertir les fr?quences en DataFrame et ajouter une colonne pour les modalit?s
freq_last_question <- as.data.frame(freq_last_question)
freq_last_question <- rownames_to_column(freq_last_question, "LastQuestion")

# Calculer la moyenne de Progress pour chaque modalit? de LastQuestion
mean_progress <- d_all %>%
...
## idem pour la question abandonn?e
...
#### Evolution de la passation et des relances ? Sciences Po ####



# Cr?er une s?quence de dates entre le 25 mars et le 20 mai (jours seulement)
...
# Joindre la s?quence de dates avec les r?sultats pour inclure les jours sans r?ponses
daily_counts <- data.frame(open_day = date_seq) %>%
  left_join(daily_counts, by = "open_day") %>%
  replace_na(list(daily_count = 0))  

# Calculer l'effectif cumul?
daily_counts <- daily_counts %>%
  mutate(cumulative_count = cumsum(daily_count))

# Calculer le pourcentage simple et cumul? du taux de r?ponses
...
# Cr?ation des groupes de dates correspondant ? diff?rents relais de relances
...
# Cr?er un dataframe pour les lignes verticales
...
# Cr?er le plot avec ggplot2
plot_scpo <- ggplot(daily_counts, aes(x = open_day)) +
  geom_line(aes(y = daily_count, color = "Effectif journalier"), size = 1) +
  geom_line(aes(y = cumulative_count, color = "Effectif cumul?"), size = 1) +
  scale_y_continuous(name = "Effectif") +
  labs(title = "Evolution de la passation ? Sciences Po",
       x = "Date",
       y = "Effectif") +
  theme_minimal(base_size = 15) +
  scale_color_manual(values = c("Effectif journalier" = "#0072B2", "Effectif cumul?" = "#D55E00")) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.title = element_blank(),
    legend.position = "bottom",
    legend.text = element_text(size = 8),  # R?duire la taille des ?tiquettes de la l?gende
  ...
  geom_vline(data = vertical_lines, aes(xintercept = as.numeric(date), color = group),
             ...
             show.legend = TRUE) +
  scale_color_manual(name = "L?gende",
                     values = c("Effectif journalier" = "#0072B2", "Effectif cumul?" = "#D55E00", 
                                "Relances Mail" = "#009E73", "Relances Campus" = "#E69F00", "Relances Flyers" = "#999999")) +
  guides(color = guide_legend(ncol = 2, byrow = TRUE))  # Diviser la l?gende en plusieurs colonnes
...
#### Evolution de la passation et des relances ? Sciences Po en fonction des campus Paris/autres####
...
# Ajouter une colonne pour diff?rencier les deux parties du graphique
...
# Cr?er un dataframe pour les lignes verticales avec linetype appropri?
vertical_lines <- data.frame(
  date = c(relances_mail, relances_campus, relances_flyers),
  group = factor(rep(c("Relances Mail", "Relances Campus", "Relances Flyers"),
                     times = c(length(relances_mail), length(relances_campus), length(relances_flyers))))
...
# Cr?er le plot avec ggplot2 pour les campus sans courbes liss?es
plot_scpo_campus <- ggplot(daily_counts_campus, aes(x = open_day, y = daily_count)) +
  ...
  scale_y_continuous(name = "Effectif") +
  labs(title = "?volution de la passation ? Sciences Po par campus",
       x = "Date",
       y = "Effectif") +
  theme_minimal(base_size = 15) +
  theme(
  ...
  scale_color_manual(name = "L?gende",
                     values = c("Campus de Paris" = "#0072B2", "Autres Campus" = "#D55E00", 
                                "Relances Mail" = "#009E73", "Relances Campus" = "#E69F00", "Relances Flyers" = "#999999")) +
  scale_linetype_manual(name = "L?gende",
...
#### M?me travail pour UPCit? ####

#### Evolution de la passation et des relances ? UPcit? ####
...
plot_upcite <- ggplot(daily_counts, aes(x = open_day)) +
  ...
  geom_line(aes(y = cumulative_count, color = "Effectif cumul?"), size = 1) +
  scale_y_continuous(name = "Effectif") +
  labs(title = "Evolution de la passation ? Universit? Paris Cit?",
       x = "Date",
       y = "Effectif") +
  theme_minimal(base_size = 15) +
  scale_color_manual(values = c("Effectif journalier" = "#0072B2", "Effectif cumul?" = "#D55E00")) +
...
#### Passation ? UPCit? par facult? ####

# Cr?er la s?quence de dates entre le 25 mars et le 20 mai (jours seulement)
date_seq <- seq(from = as.Date("2024-03-25"), to = as.Date("2024-05-20"), by = "day")

# Filtrer et pivoter les donn?es uniquement pour les facult?s 1, 2 et 3
df_combined <- d_all %>%
  pivot_longer(
...
# Calculer l'effectif simple pour chaque jour et chaque facult?
...
# Calculer le pourcentage simple du taux de r?ponses
...
# Nombre de r?pondants par facult?
echantillon <- df_combined %>%
  group_by(faculte) %>%
  summarise(repondants = n_distinct(ResponseId))

# Population par facult? (exemple fictif)
population <- data.frame(
  faculte = c("I_U_FACULTE_1", "I_U_FACULTE_2", "I_U_FACULTE_3"),
  population = c(21772, 8280, 20362)  # Remplacer par les valeurs r?elles
)

# Fusionner les informations sur les r?pondants et la population
faculte_info <- echantillon %>%
  left_join(population, by = "faculte") %>%
  mutate(taux_reponse = repondants / population * 100,
         info_label = paste0("Echantillon: ", repondants, "\nPopulation: ", population, "\nTaux de r?ponse: ", round(taux_reponse, 2), "%"))


# Custom labels for facets
facet_labels <- c(
  I_U_FACULTE_1 = "Facult? de Sant?",
  I_U_FACULTE_2 = "Facult? des Sciences",
  I_U_FACULTE_3 = "Facult? des Humanit?s"
)

# Cr?er un dataframe pour la p?riode de vacances
vacances <- data.frame(
...
plot_facultes <- ggplot(daily_counts, aes(x = open_day)) +
  ...
  geom_line(aes(y = daily_count, color = "Effectif journalier"), size = 1) +
  geom_vline(data = vertical_lines, aes(xintercept = as.numeric(date), color = group, linetype = group),
             size = 0.5, show.legend = FALSE) +  # Ne pas montrer les lignes verticales dans la l?gende
  scale_y_continuous(name = "Effectif") +
  labs(title = "Evolution de la passation ? Universit? Paris Cit? par Facult?",
       x = "Date",
       y = "Effectif") +
  theme_minimal(base_size = 15) +
  scale_color_manual(name = NULL, 
                     values = c("Effectif journalier" = "#0072B2", 
                                "Relances Mail" = "#009E73", "Relances Flyers" = "#999999")) +
  scale_fill_manual(name = NULL, values = c("P?riode de vacances" = "blue")) +
  ...
  theme(
    ...
    legend.text = element_text(size = 8),  # R?duire la taille des ?tiquettes de la l?gende
    plot.title = element_text(hjust = 0.5),
    legend.background = element_rect(fill = "white", color = NA)  # Fond blanc pour la l?gende
  ) +
  guides(color = guide_legend(override.aes = list(linetype = c(1))), 
         fill = guide_legend(override.aes = list(alpha = 0.2))) +
  facet_wrap(~ faculte, scales = "free_y", ncol = 1, labeller = as_labeller(facet_labels)) +  # Diviser les graphiques par facult?
  geom_text(data = faculte_info, aes(x = min(daily_counts$open_day), y = Inf, label = info_label),
            vjust = 1, hjust = -0.2, size = 3, inherit.aes = FALSE)  # Ajouter les annotations avec un d?calage
